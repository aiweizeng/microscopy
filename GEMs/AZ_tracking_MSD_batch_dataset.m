...
%cd
directory_path= '/Volumes/azeng/microscopy data/2024_01_17_GEM_timecourse/correct delay/';
cd 
%% 
...
% Assuming basename contains the name of the file without extension
next_folder_path = '/Volumes/azeng/microscopy data/2024_01_17_GEM_timecourse/correct delay/thunderstorm_output/'; % Specify the path to the next folder
filename = fullfile(next_folder_path, [basename, '.csv']); % Construct the full path to the file

% Open the file
out_file = cat(2,basename,'_tracksNMSD.mat'); 

% Check if the output file already exists
if exist(out_file, 'file') ~= 2  % Check if the file does not exist
    % Open the file
    fileID = fopen(filename, 'r');
...
clear X Poolfoundspots Temp trackCell i j tracks ma MSDfit 
    ...
    disp('Output file already exists. Skipping opening file.');
end
end    
